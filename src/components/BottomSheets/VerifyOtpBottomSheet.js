...
const VerifyOtpBottomSheet = props => {
  const {visibility, onBackdropPress, onCloseClick} = props;
  ...
  const onSubmitPress = async () => {
    ...
    const params = {
      email: selector.claimYourSubEmail,
      otp: selector.otp,
    };
      ...
      >
        <View
          ...
          <View style={{marginBottom: dimens.h5}}>
            ...
            {showModal ? (
              <Text style={{color: colors.warmred, marginBottom: 10}}>
                {selector?.verifyOTPRes?.message}
              </Text>
            ) : (
              <></>
            )}
            <View style={{flex: 1, marginTop: dimens.h5, alignSelf: 'center'}}>
              <RoundedButtom
